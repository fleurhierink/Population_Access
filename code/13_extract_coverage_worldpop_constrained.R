##' Title: Extracting coverage statistics for WorldPop constrained
##' Description: In this R-script we extract the accessibility coverage statistics for WorldPop constrained 
...
population <- c(list.files("data/processed/rPopulation/rPopulation_proj_corr/worldpop_constr/", recursive = T,full.names = T, pattern = "*.tif$"))
  ...
  if (size > 100) {
    ...
    worldpop_unconst_df <- final_df %>%
      mutate(wpcon_totpop = wp_con,
             ...
             wpcon_90 = ifelse(traveltime <= 90, wpcon_totpop, 0), 
             wpcon_120 = ifelse(traveltime <= 120, wpcon_totpop, 0), 
             wpcon_150 = ifelse(traveltime <= 150, wpcon_totpop, 0),
             wpcon_180 = ifelse(traveltime <= 180, wpcon_totpop, 0), 
             wpcon_barrier = ifelse(is.na(traveltime), wpcon_totpop, 0)) %>%
      select(-c(wp_con, traveltime, geometry)) %>%
      group_by(adm, GID_0, NAME_0, GID_1, NAME_1, GID_2, NAME_2) %>%
      summarise_all(.funs = sum, na.rm = T) %>%
      mutate(wpcon_30per = (wpcon_30/wpcon_totpop)*100, 
             wpcon_60per = (wpcon_60/wpcon_totpop)*100,
             wpcon_90per = (wpcon_90/wpcon_totpop)*100,
             wpcon_120per = (wpcon_120/wpcon_totpop)*100, 
