##' Title: Extracting coverage statistics for GPWv4
##' Description: In this R-script we extract the accessibility coverage statistics for GPWv4 
...
population <- c(list.files("data/processed/rPopulation/rPopulation_proj_corr/gpwv4/", recursive = T,full.names = T, pattern = "*.tif$"))
...
for (i in 1:nrow(admin0)) {
  ...
  if (size > 100) {
    ...
    for (j in 1:length(polygon)) {
      ...
      colnames(df) <- c("gpwv4", "traveltime", "adm")
    ...
    gpwv4_df <- final_df %>%
      mutate(gpwv4_totpop = gpwv4,
             gpwv4_30 = ifelse(traveltime <= 30, gpwv4_totpop, 0), 
             gpwv4_60 = ifelse(traveltime <= 60, gpwv4_totpop, 0), 
             gpwv4_90 = ifelse(traveltime <= 90, gpwv4_totpop, 0),
             gpwv4_120 = ifelse(traveltime <= 120, gpwv4_totpop, 0),
             gpwv4_150 = ifelse(traveltime <= 150, gpwv4_totpop, 0), 
             gpwv4_180 = ifelse(traveltime <= 180, gpwv4_totpop, 0), 
             gpwv4_barrier = ifelse(is.na(traveltime), gpwv4_totpop, 0)) %>%
      select(-c(gpwv4, traveltime, geometry)) %>%
      group_by(adm, GID_0, NAME_0, GID_1, NAME_1, GID_2, NAME_2) %>%
      summarise_all(.funs = sum, na.rm = T) %>%
      mutate(gpwv4_30per = (gpwv4_30/gpwv4_totpop)*100, 
             gpwv4_60per = (gpwv4_60/gpwv4_totpop)*100,
             gpwv4_90per = (gpwv4_90/gpwv4_totpop)*100, 
             gpwv4_120per = (gpwv4_120/gpwv4_totpop)*100, 
             gpwv4_150per = (gpwv4_150/gpwv4_totpop)*100, 
             gpwv4_180per = (gpwv4_180/gpwv4_totpop)*100, 
             gpwv4_barrierper = (gpwv4_barrier/gpwv4_totpop)*100)
    
    write.csv(gpwv4_df, file = paste0("data/processed/tCoverage/gpwv4/gpwv4_", iso3, ".csv"))
  } 
