##' Title: Extracting coverage statistics for GHS-POP
##' Description: In this R-script we extract the accessibility coverage statistics for GHS-POP 
...
population <- c(list.files("data/processed/rPopulation_proj_corr/ghs_pop/", recursive = T,full.names = T, pattern = "*.tif$"))
...
# we convert the population points to a dataframe in which we can summarize accessibility statistics
...
for (i in 1:nrow(admin0)) {
  ...
  if (size > 10) {
    raster <- rast(nrows = 5, ncols = 5, extent = ext(pop), crs = crs(pop), vals = seq(1:100)) # in case the population raster is too big in size, we split it in smaller chunks
    ...
    for (j in 1:length(polygon)) {
      ...
      df <- cbind(as.data.frame(pop_points), pop_tt[2]) # the two extracted values 1) travel time and 2) admin boundary are merged in one dataframe
      df <- cbind(df, pop_adm[2]) 
      colnames(df) <- c("ghs_pop", "traveltime", "adm")
    ...
    ghs_df <- final_df %>%
      mutate(ghs_totpop = ghs_pop,
             ghs_30 = ifelse(traveltime <= 30, ghs_totpop, 0), 
             ghs_60 = ifelse(traveltime <= 60, ghs_totpop, 0), 
             ghs_90 = ifelse(traveltime <= 90, ghs_totpop, 0), 
             ghs_120 = ifelse(traveltime <= 120, ghs_totpop, 0), 
             ghs_150 = ifelse(traveltime <= 150, ghs_totpop, 0),
             ghs_180 = ifelse(traveltime <= 180, ghs_totpop, 0),
             ghs_barrier = ifelse(is.na(traveltime), ghs_totpop, 0)) %>%
      select(-c(ghs_pop, traveltime, geometry)) %>%
      group_by(adm, GID_0, NAME_0, GID_1, NAME_1, GID_2, NAME_2) %>%
      summarise_all(.funs = sum, na.rm = T) %>%
      mutate(ghs_30per = (ghs_30/ghs_totpop)*100, 
             ghs_60per = (ghs_60/ghs_totpop)*100,
             ghs_90per = (ghs_90/ghs_totpop)*100,
             ghs_120per = (ghs_120/ghs_totpop)*100, 
             ghs_150per = (ghs_150/ghs_totpop)*100,
             ghs_180per = (ghs_180/ghs_totpop)*100,
             ghs_barrierper = (ghs_barrier/ghs_totpop)*100)
    write.csv(ghs_df, file = paste0("data/processed/tCoverage/ghs_pop/ghs_pop_", iso3, ".csv"))
  } 
