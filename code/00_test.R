...
download_landcover(mainPath, region)
...
download_osm("waterPolygons", mainPath, region, countryName = FALSE)
download_osm("waterLines", mainPath, region, countryName = TRUE)
download_osm("roads", mainPath, region, countryName = TRUE)

# Process the inputs (crop, mask, project, resample, etc.)
process_inputs(mainPath, region, mostRecent = FALSE)

# Tells which input folders are still empty (raw or processed)
check_input(mainPath, region, "processed" , print = TRUE)

# Check directory tree
dir_tree(paste0(mainPath, "/", region, "/data"))

# Copy all processed inputs to a "toAccessMod" folder
compile_processed_data (mainPath, region, TRUE)
