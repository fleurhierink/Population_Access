##' Title: Extracting coverage statistics for WorldPop unconstrained
##' Description: In this R-script we extract the accessibility coverage statistics for WorldPop unconstrained 
...
population <- c(list.files("data/processed/rPopulation/rPopulation_proj_corr/worldpop_unconstr/", recursive = T,full.names = T, pattern = "*.tif$"))[1:25]
traveltime <- c(list.files("data/processed/rTravel_times", full.names = T, pattern = "*.tif$", recursive = T))[1:25]
...
for (i in 1:nrow(admin0)) {
  ...
  if (size > 100) {
    ...
    for (j in 1:length(polygon)) {
      ...
      colnames(df) <- c("worldpop", "traveltime", "adm")
    ...
    worldpop_unconst_df <- final_df %>%
      mutate(wpun_totpop = worldpop,
             wpun_30 = ifelse(traveltime <= 30, wpun_totpop, 0), 
             wpun_60 = ifelse(traveltime <= 60, wpun_totpop, 0), 
             wpun_90 = ifelse(traveltime <= 90, wpun_totpop, 0),
             wpun_120 = ifelse(traveltime <= 120, wpun_totpop, 0),
             wpun_150 = ifelse(traveltime <= 150, wpun_totpop, 0),
             wpun_180 = ifelse(traveltime <= 180, wpun_totpop, 0),
             wpun_barrier = ifelse(is.na(traveltime), wpun_totpop, 0)) %>%
      select(-c(worldpop, traveltime, geometry)) %>%
      group_by(adm, GID_0, NAME_0, GID_1, NAME_1, GID_2, NAME_2) %>%
      summarise_all(.funs = sum, na.rm = T) %>%
      mutate(wpun_30per = (wpun_30/wpun_totpop)*100, 
             wpun_60per = (wpun_60/wpun_totpop)*100,
             wpun_90per = (wpun_90/wpun_totpop)*100,
             wpun_120per = (wpun_120/wpun_totpop)*100, 
             wpun_150per = (wpun_150/wpun_totpop)*100,
             wpun_180per = (wpun_180/wpun_totpop)*100,
             wpun_barrierper = (wpun_barrier/wpun_totpop)*100)
    
    write.csv(worldpop_unconst_df, file = paste0("data/processed/tCoverage/worldpop_unconstr/worldpop_unconstr_", iso3, ".csv"))
  } 
