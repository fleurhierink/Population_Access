##' Title: Extracting coverage statistics for HRSL
##' Description: In this R-script we extract the accessibility coverage statistics for HRSL 
...
population <- c(list.files("data/processed/rPopulation/rPopulation_proj_corr/facebook/", recursive = T,full.names = T, pattern = "*.tif$"))
...
for (i in 1:nrow(admin0)) {
  ...
  if (size > 100) {
    ...
    for (j in 1:length(polygon)) {
      ...
      colnames(df) <- c("facebook", "traveltime", "adm")
    ...
    facebook_df <- final_df %>%
      mutate(facebook_totpop = facebook,
             facebook_30 = ifelse(traveltime <= 30, facebook_totpop, 0), 
             facebook_60 = ifelse(traveltime <= 60, facebook_totpop, 0), 
             facebook_90 = ifelse(traveltime <= 90, facebook_totpop, 0), 
             facebook_120 = ifelse(traveltime <= 120, facebook_totpop, 0),
             facebook_150 = ifelse(traveltime <= 150, facebook_totpop, 0),
             facebook_180 = ifelse(traveltime <= 180, facebook_totpop, 0),
             facebook_barrier = ifelse(is.na(traveltime), facebook_totpop,0)) %>%
      select(-c(facebook, traveltime, geometry)) %>%
      group_by(adm, GID_0, NAME_0, GID_1, NAME_1, GID_2, NAME_2) %>%
      summarise_all(.funs = sum, na.rm = T) %>%
      mutate(facebook_30per = (facebook_30/facebook_totpop)*100, 
             facebook_60per = (facebook_60/facebook_totpop)*100,
             facebook_90per = (facebook_90/facebook_totpop)*100,
             facebook_120per = (facebook_120/facebook_totpop)*100,
             facebook_150per = (facebook_150/facebook_totpop)*100,
             facebook_180per = (facebook_180/facebook_totpop)*100,
             facebook_barrierper = (facebook_barrier/facebook_totpop)*100)
    
    write.csv(facebook_df, file = paste0("data/processed/tCoverage/facebook/facebook_", iso3, ".csv"))
  } 
